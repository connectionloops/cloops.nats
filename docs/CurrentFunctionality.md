...
- **Automatic message validation**: If a message type has a `Validate()` method, the SDK automatically validates messages before the handler runs. The handler is never invoked for an invalid payload.
- **Validation failures reach your exception handlers**: A failed `Validate()` is offered to the registered `INatsConsumerExceptionHandler`s exactly like a handler fault, so a core request/reply caller can be answered with a typed error instead of being left to time out. The context reports `IsValidationFailure = true` and carries **the exception your schema threw**, not an SDK wrapper, so a handler that already classifies its own validation exception needs no change. If no handler claims it the message is discarded exactly as before: terminated on JetStream, dropped on Core. On JetStream a validation failure is never redelivered — an ack asking for redelivery is downgraded to a terminate and logged at `Warning`, because a payload that failed validation cannot become valid on a retry.
